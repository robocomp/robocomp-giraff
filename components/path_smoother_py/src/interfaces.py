...
Ice.loadSlice("-I ./src/ --all ./src/PathSmoother.ice")
import RoboCompPathSmoother
...
class Path(list):
    ...
    def append(self, item):
        assert isinstance(item, RoboCompPathSmoother.PathPoint)
...
setattr(RoboCompPathSmoother, "Path", Path)

import pathsmootherI
...
class Implements:
    def __init__(self, ice_connector, default_handler):
        self.ice_connector = ice_connector
        self.pathsmoother = self.create_adapter("PathSmoother", pathsmootherI.PathSmootherI(default_handler))
