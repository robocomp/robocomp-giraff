...
/** \mainpage RoboComp::attention_control
 *
 * \section intro_sec Introduction
 *
 * The attention_control component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/attention_control --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class attention_control : public RoboComp::Application
{
public:
	attention_control (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
void ::attention_control::initialize()
{
...
int ::attention_control::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "EmotionalMotorProxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy EmotionalMotorProxy\n";
		}
		emotionalmotor_proxy = Ice::uncheckedCast<RoboCompEmotionalMotor::EmotionalMotorPrx>( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception creating proxy EmotionalMotor: " << ex;
		return EXIT_FAILURE;
	}
	...
	tprx = std::make_tuple(camerargbdsimple_proxy,differentialrobot_proxy,emotionalmotor_proxy,jointmotorsimple_proxy);
...
int main(int argc, char* argv[])
{
	...
	::attention_control app(prefix, startup_check_flag);
