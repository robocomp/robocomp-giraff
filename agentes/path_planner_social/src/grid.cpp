...
{
    float discarded_cost;
    return computePath(source_, target_, discarded_cost);
};

std::list<QPointF> Grid::computePath(const QPointF &source_, const QPointF &target_, float &cost)
{
    ...
    while (not active_vertices.empty())
    {
        ...
        if (where == target)
        {
            ...
            auto path = orderPath(previous, source, target);
            //qInfo() << "p.size() = " << p.size();
            //esto es solo pa cd encuentra un path eh xd
            if (path.size() > 1) {
                cost = min_distance[fmap.at(where).id]; // TODO: is this correct? dunno
                return path;
            } else {
                cost = std::numeric_limits<float>::infinity();
                return std::list<QPointF>();
